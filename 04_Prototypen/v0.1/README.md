...
Die implementierte Population 1 arbeitet dagegen mit der unveränderten zufälligen RAM-Suppe. Ihr Genom hält den Suchstand in `Z[0]`, speichert gelesene Werte in `Z[1]` und leitet Nichtnull-Werte über `GATE` nach `Z[2]` weiter. Ein getrenntes Such-und-Handshake-Fragment beginnt die Membransuche bei der vorherigen ID in der eigenen Nachbarschaft, schlägt eine tatsächlich gefundene lebende Amöbe vor, liest deren Membranwert von `0` bis `10` und kann eine im fremden Partnerslot erkannte Einladung an die eigene ID erwidern. `0` bedeutet tot, `10` energetisch und strukturell fortpflanzungsfähig; ohne ein vollständig über P-Kanten versorgtes `MEM_WRITE` bleibt der Wert auch bei beliebig hoher Energie höchstens `9`. Die Zwischenwerte bleiben bewusst unbenannt. Jede Wertänderung kann neue Erkenntnis sein. Beobachtet das Genom den Tod seines eingetragenen Partners, darf es den eigenen Slot mit diesem provenienzbehafteten Nullwert wieder freigeben. Eine errechnete oder konstante ID ohne passende Fundherkunft wird beim `MEM_WRITE` abgewiesen. Ein drittes Fragment schreibt die eigene ID neutral nach `RAM[eigene ID]`; Schreiben selbst erzeugt keine Energie.
...
Zusätzlich führt die Chronik historische Amöbenrekorde: größtes Genom, höchste jemals beobachtete Energie, direkte Kinder, gesamte Nachkommenschaft, Generationstiefe, Alter, erkundete RAM-Adressen, persönlicher RAM-Energiegewinn und erfolgreichster Informationsproduzent. Verhaltensrekorde erfassen außerdem gefundene Amöben, erkannte Einladungen und RAM-Schreibvorgänge. Direkte Kinder und gesamte Nachkommenschaft bleiben bewusst getrennt; letztere umfasst auch Enkel und alle weiteren erreichbaren Generationen. Beim Informationsproduzenten wird eine ausgezahlte Leseenergie gleichmäßig auf die in der Herkunftskette des RAM-Werts enthaltenen Urheber verteilt; Eigenfütterung bleibt wie im Core ausgeschlossen.
