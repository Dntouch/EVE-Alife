...
</style><link rel="stylesheet" href="/assets/v03.css?v=6"></head><body><main><div class="top"><div><h1>EVE-Alife · Lupe</h1><div class="subtitle">Read-only-Beobachtung eines laufenden oder archivierten Experiments</div></div><label>Zeitpunkt ansehen<br><select id="snap"><option value="latest">aktuell</option></select></label></div>
...
const setRamZoom=(factor,center=ramCenter)=>{ramZoom=Math.max(1,Math.min(128,ramZoom*factor));ramCenter=((center%liveRamSize)+liveRamSize)%liveRamSize;if(window.eveV03)window.eveV03.updateZoom(ramZoom)};
window.addEventListener('eve:ramzoom',event=>setRamZoom(event.detail));
window.addEventListener('eve:ramreset',()=>{ramZoom=1;ramCenter=liveRamSize/2;if(window.eveV03)window.eveV03.updateZoom(ramZoom)});
...
soupCanvas.addEventListener('click',()=>{if(suppressSoupClick){suppressSoupClick=false;return}if(!hoveredSoupCluster)return;if(hoveredSoupCluster.entities.length>1){const positions=hoveredSoupCluster.entities.map(e=>Number(e.position)),unique=new Set(positions);if(unique.size===1||ramZoom>=128){if(window.eveV03)window.eveV03.showCluster(hoveredSoupCluster.entities.map(entity=>({...entity,name:entity.name||nameOf(entity.id)})));return}setRamZoom(2,positions.reduce((a,b)=>a+b,0)/positions.length);return}const entity=hoveredSoupCluster.entities[0];if(window.eveV03)window.eveV03.showEntity({...entity,name:entity.name||nameOf(entity.id)})});
soupCanvas.addEventListener('wheel',event=>{event.preventDefault();const rect=soupCanvas.getBoundingClientRect(),px=(event.clientX-rect.left)*soupCanvas.width/rect.width,pad=58,span=liveRamSize/ramZoom,address=ramCenter+(px-pad)/(soupCanvas.width-pad*2)*span-span/2;setRamZoom(event.deltaY<0?1.5:1/1.5,address)},{passive:false});
...
</script><script src="/assets/v03.js?v=6"></script></body></html>'''
...
def handler_for(default_run: Path, catalog_roots: tuple[Path, ...] = ()):
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            assets = {
                "/assets/v03.css": ("v03.css", "text/css; charset=utf-8"),
                "/assets/v03.js": ("v03.js", "text/javascript; charset=utf-8"),
                "/assets/eve-e-logo.png": ("eve-e-logo.png", "image/png"),
                "/assets/eve-v03-biotope-wallpaper.png": ("eve-v03-biotope-wallpaper.png", "image/png"),
            }
            if path in assets:
...
def main() -> None:
    ...
    v03_runs = Path(__file__).resolve().parent / "runs"
    v02_runs = Path(__file__).resolve().parent.parent / "v0.2" / "runs"
    server = ThreadingHTTPServer(("127.0.0.1", args.port), handler_for(source, (v03_runs, v02_runs)))
