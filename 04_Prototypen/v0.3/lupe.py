...
</style><link rel="stylesheet" href="/assets/v03.css?v=8"></head><body><main><div class="top"><div><h1>EVE-Alife · Lupe</h1><div class="subtitle">Read-only-Beobachtung eines laufenden oder archivierten Experiments</div></div><label>Zeitpunkt ansehen<br><select id="snap"><option value="latest">aktuell</option></select></label></div>
...
const setRamZoom=(factor,center=ramCenter)=>{ramZoom=Math.max(1,Math.min(128,ramZoom*factor));ramCenter=((center%liveRamSize)+liveRamSize)%liveRamSize;if(window.eveV03)window.eveV03.updateZoom(ramZoom);updateRamNavigator()};
window.addEventListener('eve:ramzoom',event=>setRamZoom(event.detail));
window.addEventListener('eve:ramreset',()=>{ramZoom=1;ramCenter=liveRamSize/2;if(window.eveV03)window.eveV03.updateZoom(ramZoom);updateRamNavigator()});
...
</script><script src="/assets/v03.js?v=8"></script></body></html>'''
...
def main() -> None:
    ...
    server = ThreadingHTTPServer((args.host, args.port), handler_for(source, (v03_runs, v02_runs)))
