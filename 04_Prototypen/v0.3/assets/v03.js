(() => {
  ...
  historyContext.onclick=()=>{window.eveV03.setView('live');requestAnimationFrame(()=>document.querySelector('#timeline-panel')?.scrollIntoView({behavior:'smooth',block:'center'}))};
  ...
  window.eveV03 = {
    ...
    async showGenomeById(entityId) {
      const cached = window.eveV03Entities?.find(item => Number(item.id) === Number(entityId));
    ...
    async showGenomeComparison(childId, parentId) {
      ...
      try {
        ...
        const parentOptions=(data.parents||[]).map(id=>{const known=window.eveV03Entities?.find(entity=>Number(entity.id)===Number(id));return`<option value="${id}" ${Number(id)===Number(parentId)?'selected':''}>${safe(known?.name||`Amöbe #${id}`)} · #${id}</option>`}).join('');
        ...
        workspace.querySelector('[data-compare-parent]').onchange=event=>this.showGenomeComparison(childId,Number(event.target.value));workspace.querySelector('[data-compare-close]').onclick=()=>{const entity=window.eveV03Entities?.find(item=>Number(item.id)===Number(childId));if(entity)this.showGenome(entity)};requestAnimationFrame(()=>window.scrollTo({top:0,behavior:'auto'}));
  ...
  function renderLineage(root, data) {
    ...
    inspector.addEventListener('click',event=>{const button=event.target.closest('[data-open-history]');if(!button||!focusId)return;event.preventDefault();event.stopImmediatePropagation();sessionStorage.setItem('eve-open-entity',String(focusId));window.eveV03.setView('history');const snapshot=document.querySelector('#snap');if(snapshot){snapshot.value='latest';snapshot.dispatchEvent(new Event('change'))}},true);
