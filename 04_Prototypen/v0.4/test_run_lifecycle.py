...
class RunLifecycleTests(unittest.TestCase):
    ...
    def test_v04_lupe_retains_analysis_and_adds_separate_genome_workspace(self) -> None:
        ...
        self.assertIn("Genom-Analyse öffnen", HTML)
        self.assertIn('/assets/v04.css', HTML)
        self.assertIn('/assets/v04.js', HTML)
        ...
        ui_script = (HERE / "assets" / "v04.js").read_text(encoding="utf-8")
        ui_style = (HERE / "assets" / "v04.css").read_text(encoding="utf-8")
        self.assertIn("<h2>Leitstand</h2>", ui_script)
        self.assertIn("/api/runs", ui_script)
        self.assertIn("127.0.0.1:8767/api/start", ui_script)
        self.assertIn("Historische Analyse", ui_script)
        ...
        self.assertIn("eve-v04-biotope-wallpaper.png", ui_style)
    ...
    def test_chronicle_summarizes_runs_across_roots(self) -> None:
        _, first = self.run_cli("--ticks", "1", "--seed", "3")
        _, second = self.run_cli("--ticks", "2", "--seed", "4")
        chronicle = chronicle_data((first.parent, second.parent))
        self.assertEqual(chronicle["totals"]["runs"], 2)
        self.assertEqual(len(chronicle["runs"]), 2)
        self.assertEqual(chronicle["records"]["longest_run"]["ticks"], 2)
    ...
    def test_genomes_are_deduplicated_and_ancestry_is_queryable(self) -> None:
        ...
        manifest = {
            "run_id": "test", "format_version": 2, "prototype_version": "0.4",
