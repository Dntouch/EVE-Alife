(() => {
  ...
  rail.querySelector('.eve-nav').append(rail.querySelector('[data-view="chronicle"]'));
  ...
  lineageView.innerHTML = `<header class="eve-view-header"><h2>Evolutionäre Zeitlandschaft</h2><p>Abstammung, Selektion und Genomveränderung der gewählten Amöbe.</p></header><div id="lineage-workspace" class="lineage-loading panel">Erst Amöbe in der Historie wählen.</div>`;
  ...
  if(timelinePanel&&timelineCanvas){const heading=timelinePanel.querySelector('h2'),copy=timelinePanel.querySelector('.muted');heading.textContent='Evolutionsverlauf';copy.textContent='Population, Genomvielfalt und Schlüsselereignisse. Ziehen wählt aus, Strg/Cmd + Mausrad zoomt, Umschalt-Ziehen verschiebt.';const controls=document.createElement('div');controls.className='evolution-quick-actions';controls.innerHTML='<button type="button" data-evolution-range="all">Gesamt</button><button type="button" data-evolution-range="last">Letzte 100 Ticks</button><button type="button" data-evolution-range="peak">Populationsmaximum</button><button type="button" data-evolution-range="change">Stärkste Veränderung</button><span class="evolution-refresh"><button type="button" data-evolution-refresh title="Nur neue Messpunkte ergänzen">↻ Neue Ticks</button><small data-evolution-refresh-state>inkrementell</small></span>';timelinePanel.querySelector('.top').append(controls);const legend=document.createElement('div');legend.className='evolution-legend';legend.innerHTML='<span class="population"><i></i>Population</span><span class="genomes"><i></i>Genomvielfalt</span><span class="birth"><i></i>Geburt</span><span class="death"><i></i>Tod</span><span class="milestone"><i></i>Schlüsselereignis</span>';const stage=document.createElement('div');stage.className='evolution-chart-stage';timelineCanvas.before(legend);timelineCanvas.replaceWith(stage);stage.append(timelineCanvas);stage.insertAdjacentHTML('beforeend','<div id="timeline-tooltip" class="evolution-tooltip" hidden></div>');controls.querySelector('[data-evolution-refresh]').onclick=()=>window.dispatchEvent(new Event('eve:evolution-refresh'));controls.querySelectorAll('[data-evolution-range]').forEach(button=>button.onclick=()=>{const kind=button.dataset.evolutionRange,full=maxTick();if(kind==='all'){timelineViewport={start:0,end:full};selection={mode:'all',start:0,end:full}}else if(kind==='last'){const start=Math.max(0,full-100);timelineViewport={start,end:full};selection={mode:'range',start,end:full}}else{const target=kind==='peak'?[...timelinePoints].sort((a,b)=>b.alive-a.alive)[0]?.tick:(timelineMilestones.find(item=>item.kind==='genome_jump')?.tick??full),span=Math.min(100,full);timelineViewport={start:Math.max(0,target-span/2),end:Math.min(full,target+span/2)};selection={mode:'point',start:target,end:target}}applySelection()})}
  ...
  if (soupHead) {
    ...
    toolbar.innerHTML = '<button type="button" data-ram-zoom="out" title="Herauszoomen">−</button><button type="button" data-ram-zoom="in" title="Hineinzoomen">+</button><button type="button" data-ram-reset>Gesamt</button><span class="ram-zoom-label">Zoom 1×</span><label>Bild alle <select data-ram-tick-interval><option>5</option><option>10</option><option selected>25</option><option>50</option><option>100</option></select> Ticks</label><span class="ram-navigation-help">Ziehen: bewegen · Strg + Mausrad: zoomen · Doppelklick: fokussieren</span><span class="ram-minimap" aria-label="Position im gesamten RAM"><i></i></span><span class="ram-ring" aria-hidden="true"></span><span class="ram-ring-label">zyklischer RAM</span>';
    const interval=toolbar.querySelector('[data-ram-tick-interval]'),saved=localStorage.getItem('eve-soup-ticks')||'25';interval.value=[...interval.options].some(option=>option.value===saved)?saved:'25';interval.onchange=()=>window.eveSetSoupTickInterval?.(interval.value);
  ...
  let lastDataSignal = 0, selectedEntityId = null;
  window.eveV04 = {
    selectEntity(entityId) {
      selectedEntityId=Number(entityId);
      selectedLineageId=selectedEntityId;
      document.dispatchEvent(new CustomEvent('eve:entity-selected',{detail:{entityId:selectedEntityId}}));
    },
    setView(name) {
    ...
    showLineageById(entityId) {
      selectedLineageId=Number(entityId);
      this.setView('lineage');
      initLineage(true,selectedLineageId);
    },
    showGenome(entity) {
      ...
      const slotPalette=['#32bfff','#b26cff','#20e6a8','#ffb84d','#ff668f','#69e6ff','#d5ed68','#ff8ce6'];
      const slotIds=[...new Set(entity.genome.nodes.map(node=>Number(node.segment)||1))].sort((a,b)=>a-b),slotOf=new Map(entity.genome.nodes.map(node=>[Number(node.id),Number(node.segment)||1]));
      const slotSummary=slotIds.map((slot,index)=>{const nodes=entity.genome.nodes.filter(node=>(Number(node.segment)||1)===slot),internal=entity.genome.edges.filter(edge=>slotOf.get(Number(edge.source))===slot&&slotOf.get(Number(edge.target))===slot).length,outgoing=entity.genome.edges.filter(edge=>slotOf.get(Number(edge.source))===slot&&slotOf.get(Number(edge.target))!==slot).length;return{slot,nodes:nodes.length,internal,outgoing,color:slotPalette[index%slotPalette.length]}});
      const displayGenome={...entity.genome,nodes:[...entity.genome.nodes].sort((a,b)=>(Number(a.segment)||1)-(Number(b.segment)||1)||Number(a.id)-Number(b.id))};
      const edgeWeights=entity.genome.edges.map(edge=>Number(edge.weight||0)),weightedEdges=edgeWeights.filter(weight=>weight!==0),minWeight=Math.min(...edgeWeights,0),maxWeight=Math.max(...edgeWeights,0);
      workspace.innerHTML = `<header><div><small class="muted">EVE · GENOMISCHE TOPOLOGIE · AMÖBE #${entity.id}</small><h3>${entity.name || `Amöbe #${entity.id}`} · Genom</h3><div class="genome-stats"><span class="genome-stat genome-total">Gesamtgröße <b>${entity.genome.nodes.length + entity.genome.edges.length} G</b></span><span class="genome-stat genome-slots">Vererbungsplätze <b>${slotIds.length}</b></span><span class="genome-stat">Funktionspunkte <b>${entity.genome.nodes.length}</b></span><span class="genome-stat">Verbindungen <b>${entity.genome.edges.length}</b></span><span class="genome-stat genome-weighted">Gewichtet <b>${weightedEdges.length}</b></span><span class="genome-stat">Gewichtsspanne <b>${minWeight} … ${maxWeight>=0?'+':''}${maxWeight}</b></span><span class="genome-stat">Fragmente <b>${fragments}</b></span><span class="genome-stat">A₀ <b>${entity.genome.activity_base}</b></span><span class="genome-stat">Nₖ <b>${entity.genome.knock_capacity || 1}</b></span><span class="genome-stat">Tₚ <b>${entity.genome.bond_ticks || 1}</b></span></div></div><div class="genome-toolbar"><button type="button" data-genome-zoom="out" title="Herauszoomen">−</button><button type="button" data-genome-zoom="in" title="Hineinzoomen">+</button><button type="button" data-genome-reset>Gesamt</button><span class="genome-zoom-label">Zoom 1×</span><button type="button" data-genome-popup>↗ Separates Fenster</button><button type="button" data-analysis-focus>⛶ Vergrößern</button></div></header><div class="genome-slot-legend"><small>VERERBUNGSARCHITEKTUR</small>${slotSummary.map(item=>`<button type="button" data-genome-slot="${item.slot}" style="--slot-color:${item.color}"><i></i><b>Platz ${item.slot}</b><span>${item.nodes} F · ${item.internal} intern · ${item.outgoing} Anschluss</span></button>`).join('')}</div><div class="genome-legend"><span class="read"><i></i>Lesen</span><span class="write"><i></i>Schreiben</span><span class="logic"><i></i>Logik</span><span class="const"><i></i>Konstanten</span><span class="process"><i></i>Verarbeitung</span><span class="edge-amplified"><i></i>verstärkt</span><span class="edge-damped"><i></i>gedämpft</span><span class="edge-inverted"><i></i>invertiert</span><strong class="genome-focus-label">Platz, Modul oder Kante anklicken</strong></div><div class="genome-analysis-body"><div class="genome-scroll">${window.genomeSvg(displayGenome)}</div><aside class="genome-node-inspector"><div class="genome-node-empty"><span>⌬</span><b>Funktionspunkt oder Kante auswählen</b><p>Hier erscheinen Platz, Funktion, Übertragungsgewicht und strukturelle Wirkung.</p></div></aside></div>`;
      ...
      const svgNs='http://www.w3.org/2000/svg';nodes.forEach(nodeElement=>{const data=nodeData.get(nodeElement.dataset.node),slot=Number(data?.segment)||1,item=slotSummary.find(entry=>entry.slot===slot),rect=nodeElement.querySelector(':scope > rect');nodeElement.dataset.slot=String(slot);nodeElement.style.setProperty('--slot-color',item?.color||slotPalette[0]);if(rect){const frame=document.createElementNS(svgNs,'rect');['x','y','width','height','rx'].forEach(name=>frame.setAttribute(name,rect.getAttribute(name)));frame.setAttribute('class','slot-frame');frame.setAttribute('x',String(Number(rect.getAttribute('x'))-4));frame.setAttribute('y',String(Number(rect.getAttribute('y'))-4));frame.setAttribute('width',String(Number(rect.getAttribute('width'))+8));frame.setAttribute('height',String(Number(rect.getAttribute('height'))+8));frame.setAttribute('rx','14');nodeElement.insertBefore(frame,rect);const label=document.createElementNS(svgNs,'text');label.setAttribute('class','node-slot');label.setAttribute('x',String(Number(rect.getAttribute('x'))+Number(rect.getAttribute('width'))-8));label.setAttribute('y',String(Number(rect.getAttribute('y'))+11));label.textContent=`P${slot}`;nodeElement.append(label)}});
      edges.forEach(edgeElement=>{const sourceSlot=slotOf.get(Number(edgeElement.dataset.source))||1,targetSlot=slotOf.get(Number(edgeElement.dataset.target))||1,sourceColor=slotSummary.find(item=>item.slot===sourceSlot)?.color||slotPalette[0],targetColor=slotSummary.find(item=>item.slot===targetSlot)?.color||slotPalette[0],cross=sourceSlot!==targetSlot;edgeElement.dataset.sourceSlot=String(sourceSlot);edgeElement.dataset.targetSlot=String(targetSlot);edgeElement.classList.add(cross?'slot-connector':'slot-internal');edgeElement.style.setProperty('--slot-color',sourceColor);edgeElement.style.setProperty('--target-slot-color',targetColor)});
      const descriptions={
      ...
      const emptyInspector=()=>{inspector.innerHTML='<div class="genome-node-empty"><span>⌬</span><b>Platz, Funktionspunkt oder Kante auswählen</b><p>Hier erscheinen Vererbungsarchitektur, Funktion, Übertragungsgewicht und strukturelle Wirkung.</p></div>'};
      const resetFocus = () => { svg.classList.remove('has-genome-focus'); nodes.forEach(node => node.classList.remove('is-selected','is-related','is-dimmed')); edges.forEach(edge => edge.classList.remove('is-selected','is-related','is-dimmed')); focusLabel.textContent='Modul oder Kante anklicken';emptyInspector(); };
      const focusSlot=slot=>{const item=slotSummary.find(entry=>entry.slot===slot);nodes.forEach(node=>{const active=Number(node.dataset.slot)===slot;node.classList.toggle('is-related',active);node.classList.toggle('is-selected',false);node.classList.toggle('is-dimmed',!active)});edges.forEach(edge=>{const internal=Number(edge.dataset.sourceSlot)===slot&&Number(edge.dataset.targetSlot)===slot,connector=Number(edge.dataset.sourceSlot)===slot||Number(edge.dataset.targetSlot)===slot,active=internal||connector;edge.classList.toggle('is-related',active);edge.classList.toggle('is-selected',false);edge.classList.toggle('is-dimmed',!active)});svg.classList.add('has-genome-focus');focusLabel.textContent=`Vererbungsplatz ${slot}`;inspector.innerHTML=`<small class="genome-node-kicker">EVE · INHERITANCE SLOT</small><h3>Platz ${slot}</h3><div class="genome-slot-card" style="--slot-color:${item?.color||slotPalette[0]}"><i></i><span><small>Funktionspunkte</small><b>${item?.nodes||0}</b></span><span><small>Interne Kanten</small><b>${item?.internal||0}</b></span><span><small>Anschlusskanten</small><b>${item?.outgoing||0}</b></span></div><p class="genome-node-description">Dieser Platz wird als vollständige elterliche Variante vererbt. Seine internen Kanten bleiben erhalten; Anschlusskanten verbinden ihn mit anderen Plätzen.</p>`};
      workspace.querySelectorAll('[data-genome-slot]').forEach(button=>button.onclick=()=>focusSlot(Number(button.dataset.genomeSlot)));
      const focusNode = node => {
        ...
        const link=(edge,direction)=>{const other=nodeData.get(String(direction==='in'?edge.source:edge.target)),weight=Number(edge.weight||0),gain=100+weight,otherSlot=Number(other?.segment)||1,cross=otherSlot!==(Number(selected.segment)||1);return other?`<button type="button" data-inspect-node="${other.id}"><small>${direction==='in'?`Eingang ${edge.target_port} · von`:`Ausgang ${edge.source_port} · zu`} · ${cross?`Platzübergang P${selected.segment}↔P${otherSlot}`:`intern P${selected.segment}`} · Gewicht ${weight>=0?'+':''}${weight}</small><b>#${other.id} · ${other.kind}</b><span>${direction==='out'?consequence(other,edge):'liefert das Eingangssignal'} · ${gain} % Übertragung</span></button>`:''};
      ...
      const focusEdge=edgeElement=>{const edge=entity.genome.edges[Number(edgeElement.dataset.edge)],weight=Number(edge?.weight||0),gain=100+weight,source=nodeData.get(String(edge.source)),target=nodeData.get(String(edge.target)),sourceSlot=Number(source?.segment)||1,targetSlot=Number(target?.segment)||1,cross=sourceSlot!==targetSlot,effect=weight===0?'neutral und entspricht dem bisherigen Transport':weight===-100?'blockiert den Zahlenwert vollständig':weight<-100?'kehrt das Vorzeichen um und überträgt den invertierten Rest':weight>0?'verstärkt den transportierten Zahlenwert':'dämpft den transportierten Zahlenwert';nodes.forEach(node=>{const active=node.dataset.node===String(edge.source)||node.dataset.node===String(edge.target);node.classList.toggle('is-related',active);node.classList.toggle('is-selected',false);node.classList.toggle('is-dimmed',!active)});edges.forEach(item=>{const selected=item===edgeElement;item.classList.toggle('is-selected',selected);item.classList.toggle('is-related',selected);item.classList.toggle('is-dimmed',!selected)});svg.classList.add('has-genome-focus');focusLabel.textContent=`Kante #${Number(edgeElement.dataset.edge)+1} · ${cross?`P${sourceSlot} → P${targetSlot}`:`intern P${sourceSlot}`} · Gewicht ${weight>=0?'+':''}${weight}`;inspector.innerHTML=`<small class="genome-node-kicker">EVE · EDGE TRACE · ${cross?'ANSCHLUSSKANTE':'INTERNE KANTE'}</small><h3>Kante #${Number(edgeElement.dataset.edge)+1}</h3><div class="genome-edge-places"><span style="--slot-color:${slotSummary.find(item=>item.slot===sourceSlot)?.color}"><i></i>Platz ${sourceSlot}</span><b>→</b><span style="--slot-color:${slotSummary.find(item=>item.slot===targetSlot)?.color}"><i></i>Platz ${targetSlot}</span></div><div class="genome-edge-weight ${weight>0?'positive':weight<0?'negative':'neutral'}"><small>Erbliches Gewicht</small><b>${weight>=0?'+':''}${weight}</b><span>${gain} % Übertragung</span></div><div class="node-diff"><span><small>Quelle · Port ${edge.source_port}</small><b>#${edge.source} · ${source?.kind||'?'}</b></span><i>→</i><span><small>Ziel · Port ${edge.target_port}</small><b>#${edge.target} · ${target?.kind||'?'}</b></span></div><p class="genome-node-description">${cross?`Diese Anschlusskante verbindet Vererbungsplatz ${sourceSlot} mit Platz ${targetSlot}.`:`Diese Kante bleibt vollständig innerhalb von Vererbungsplatz ${sourceSlot}.`} Sie ist ${effect}. Zur Laufzeit gilt <code>Wert × (1 + Gewicht / 100)</code>.</p><div class="genome-consequence"><small>HAT STRUKTURELL ZUR FOLGE, DASS</small><strong>${target?consequence(target,edge):'das Signal an den Zielport weitergegeben wird'}.</strong><p>Das Gewicht verändert ausschließlich den Zahlenwert. Quellen- und Urheberinformationen bleiben erhalten.</p></div>`};
      ...
      svg.addEventListener('pointerdown',event=>{if(event.target.closest?.('[data-node],[data-edge]'))return;genomeDrag={pointerId:event.pointerId,x:event.clientX,y:event.clientY,startX:view.x,startY:view.y,moved:false};svg.setPointerCapture(event.pointerId);svg.classList.add('is-panning');event.preventDefault()});
      ...
      svg.addEventListener('click',event=>{if(suppressGenomeClick){suppressGenomeClick=false;return}if(event.target.closest?.('[data-node],[data-edge]'))return;resetFocus()});
      this.setView('genome');
    },
    async showGenomeComparison(childId, parentId) {
      ...
      try {
        ...
        const parentNodes=new Map(data.parent.genome.nodes.map(node=>[Number(node.id),node])),allParentNodes=new Map(Object.entries(data.parent_genomes||{}).map(([id,item])=>[Number(id),new Map(item.genome.nodes.map(node=>[Number(node.id),node]))])),childNodes=new Map(data.child.genome.nodes.map(node=>[Number(node.id),node])),fragments=data.trace.inherited_fragments||[],origin=new Map(),selectedSources=new Set(),selectedEdgeKeys=new Set(),selectedEdgeWeights=new Map(),selectedOriginalEdgeKeys=new Set(),reconnectedEdgeKeys=new Set(),otherEdgeKeys=new Set(),mappingsByParent=new Map();
        const edgeKey=edge=>`${edge.source}.${edge.source_port}>${edge.target}.${edge.target_port}`;
        fragments.forEach(fragment=>{const parent=Number(fragment.parent_id),mapping=mappingsByParent.get(parent)||new Map();Object.entries(fragment.node_mapping||{}).forEach(([source,target])=>{mapping.set(Number(source),Number(target));origin.set(Number(target),{parentId:parent,source:Number(source)});if(parent===Number(parentId))selectedSources.add(Number(source))});mappingsByParent.set(parent,mapping)});
        fragments.forEach(fragment=>{const parent=Number(fragment.parent_id),mapping=mappingsByParent.get(parent)||new Map(),resolutions=fragment.edge_resolutions||null;if(resolutions){resolutions.forEach(item=>{const original=item.original,resolved=item.resolved;if(parent===Number(parentId))selectedOriginalEdgeKeys.add(edgeKey(original));if(!resolved)return;const key=edgeKey(resolved);if(parent===Number(parentId)){selectedEdgeKeys.add(key);selectedEdgeWeights.set(key,Number(original.weight||0));if(item.status==='reconnected')reconnectedEdgeKeys.add(key)}else otherEdgeKeys.add(key)})}else{(fragment.edges||[]).forEach(edge=>{const mapped={source:mapping.get(Number(edge.source)),source_port:edge.source_port,target:mapping.get(Number(edge.target)),target_port:edge.target_port,weight:Number(edge.weight||0)};if(mapped.source&&mapped.target){if(parent===Number(parentId)){selectedEdgeKeys.add(edgeKey(mapped));selectedEdgeWeights.set(edgeKey(mapped),mapped.weight);selectedOriginalEdgeKeys.add(edgeKey(edge))}else otherEdgeKeys.add(edgeKey(mapped))}})}});
        ...
        const lostNodes=data.parent.genome.nodes.filter(node=>!selectedSources.has(Number(node.id))),edgeStatus=new Map();data.child.genome.edges.forEach(edge=>{const key=edgeKey(edge);edgeStatus.set(key,selectedEdgeKeys.has(key)?(reconnectedEdgeKeys.has(key)||Number(edge.weight||0)!==selectedEdgeWeights.get(key)?'modified':'inherited'):otherEdgeKeys.has(key)?'other':'new')});const lostEdges=data.parent.genome.edges.filter(edge=>!selectedOriginalEdgeKeys.has(edgeKey(edge)));
        ...
        const paths=data.child.genome.edges.map(edge=>{const a=positions.get(Number(edge.source)),b=positions.get(Number(edge.target));if(!a||!b)return'';const status=edgeStatus.get(edgeKey(edge))||'new',bend=Math.max(25,Math.abs(b.y-a.y)*.4),weight=Number(edge.weight||0);return`<path class="compare-edge ${status}" data-compare-edge="${safe(edgeKey(edge))}" data-status="${status}" d="M${a.x} ${a.y+25} C${a.x} ${a.y+bend} ${b.x} ${b.y-bend} ${b.x} ${b.y-25}"><title>Gewicht ${weight>=0?'+':''}${weight} · ${100+weight} %</title></path>`}).join('');
        ...
        const counts={inherited:[...nodeStatus.values()].filter(value=>value==='inherited').length,modified:[...nodeStatus.values()].filter(value=>value==='modified').length,modifiedEdges:[...edgeStatus.values()].filter(value=>value==='modified').length,other:[...nodeStatus.values()].filter(value=>value==='other').length,new:[...nodeStatus.values()].filter(value=>value==='new').length,lost:lostNodes.length};
        workspace.innerHTML=`<header class="genome-compare-head"><div><small>EVE · GENETIC INHERITANCE</small><h3>${safe(data.parent.name)} <span>→</span> ${safe(data.child.name)}</h3><p>Direkter Eltern-Kind-Vergleich · ${data.evidence==='recorded'?'Herkunft aus protokollierter Rekombination':'Herkunft strukturell abgeleitet'}</p></div><div><label>Elternteil<select data-compare-parent>${parentOptions}</select></label><button data-compare-close>Einzelgenom</button></div></header><section class="genome-delta-strip"><div><small>Genomgröße</small><b>${data.parent.n_g} → ${data.child.n_g}</b><span>${data.child.n_g-data.parent.n_g>=0?'+':''}${data.child.n_g-data.parent.n_g}</span></div><div class="inherited"><small>Unverändert geerbt</small><b>${counts.inherited}</b></div><div class="modified"><small>Veränderte Module</small><b>${counts.modified}</b></div><div class="modified"><small>Veränderte Gewichte</small><b>${counts.modifiedEdges}</b></div><div class="other"><small>Anderer Elternteil</small><b>${counts.other}</b></div><div class="new"><small>Neu</small><b>${counts.new}</b></div><div class="lost"><small>Nicht übernommen</small><b>${counts.lost}</b></div></section><div class="genome-compare-toolbar"><div class="genome-compare-legend"><button class="active inherited" data-compare-filter="inherited"><i></i>unverändert</button><button class="active modified" data-compare-filter="modified"><i></i>verändert</button><button class="active other" data-compare-filter="other"><i></i>anderer Elternteil</button><button class="active new" data-compare-filter="new"><i></i>neu</button><button class="active lost" data-compare-filter="lost"><i></i>verloren</button></div><span>Klick auf ein Modul oder eine Kante öffnet den Herkunftsbefund</span></div><div class="genome-compare-layout"><div class="genome-compare-stage"><svg class="genome-compare-graph" viewBox="0 0 ${width} ${height}" role="img"><defs><pattern id="compare-grid" width="32" height="32" patternUnits="userSpaceOnUse"><path d="M32 0H0V32" fill="none" stroke="rgba(65,157,222,.075)"/></pattern></defs><rect width="100%" height="100%" fill="url(#compare-grid)"/><g class="compare-edges">${paths}</g><g class="compare-nodes">${boxes}</g></svg><section class="lost-genome-fragments"><header><small>NICHT IN DAS KIND ÜBERNOMMEN</small><span>${lostNodes.length} Funktionspunkte · ${lostEdges.length} Verbindungen</span></header><div>${lostNodes.map(node=>`<button data-lost-node="${node.id}" data-status="lost"><b>#${node.id} ${safe(node.kind)}</b><small>${node.constant===null?'ohne erblichen Wert':`Wert ${safe(node.constant)}`}</small></button>`).join('')||'<p class="empty">Das gesamte Elterngenom wurde übernommen.</p>'}</div></section></div><aside class="genome-diff-inspector"><div class="genome-diff-empty"><span>⌬</span><b>Unterschied auswählen</b><p>Die Karte zeigt das Genom des Kindes. Nicht übernommene Teile stehen darunter.</p></div></aside></div>`;
        ...
        workspace.querySelectorAll('[data-compare-edge]').forEach(path=>path.onclick=()=>{const key=path.dataset.compareEdge,edge=data.child.genome.edges.find(item=>edgeKey(item)===key),before=selectedEdgeWeights.get(key),after=Number(edge?.weight||0),status=path.dataset.status,reconnected=reconnectedEdgeKeys.has(key),weightChanged=before!==undefined&&before!==after;inspector.innerHTML=`<small>EVE · EDGE INHERITANCE</small><h3>${reconnected?'Anschlusskante neu gekoppelt':weightChanged?'Kantengewicht verändert':'Vererbte Kante'}</h3><div class="diff-status ${status}">${safe(key)}</div><div class="node-diff"><span><small>Elterngewicht</small><b>${before===undefined?'—':`${before>=0?'+':''}${before}`}</b></span><i>→</i><span><small>Kindgewicht</small><b>${after>=0?'+':''}${after}</b></span></div><p>${reconnected?`Das ursprüngliche Zielsegment fehlte. Die geerbte Anschlusskante wurde an diesen Eingangsport eines anderen Segments gekoppelt${weightChanged?` und ihr Gewicht änderte sich um ${after-before>=0?'+':''}${after-before}`:''}.`:weightChanged?`Die Verbindung wurde geerbt und ihr Gewicht bei der Geburt um ${after-before>=0?'+':''}${after-before} verändert.`:status==='inherited'?'Verbindung und Gewicht wurden unverändert übernommen.':'Die Kante stammt nicht nachweislich vom gewählten Elternteil.'}</p><dl><div><dt>Übertragung Kind</dt><dd>${100+after} %</dd></div><div><dt>Mutation</dt><dd>${mutation?.class==='edge_weight'?'protokollierte Gewichtsmutation':'keine passende Einzelmutation protokolliert'}</dd></div></dl>`});
    ...
    renderEntityBrowser(entities, target) {
      ...
      if (entities.length > 1000) entities = [...entities.slice(0, 500), ...entities.slice(-500)];
      ...
      const reproductionState=entity=>{if(typeof entity.reproduction_operational==='boolean'){const operational=entity.reproduction_operational;return{operational,label:operational?'fortpflanzungsfähig':'steril',reason:operational?'vollständig verdrahtetes MEM_WRITE':'kein vollständig verdrahtetes MEM_WRITE'}}const genome=entity.genome||{},edges=genome.edges||[],writers=(genome.nodes||[]).filter(node=>node.kind==='MEM_WRITE'),required=['offset','slot','value'],operational=writers.some(node=>{const ports=new Set(edges.filter(edge=>Number(edge.target)===Number(node.id)).map(edge=>String(edge.target_port)));return required.every(port=>ports.has(port))});return{operational,label:operational?'fortpflanzungsfähig':'steril',reason:operational?'vollständig verdrahtetes MEM_WRITE':'kein vollständig verdrahtetes MEM_WRITE'}};
      ...
      if (totalEntities > entities.length) {
        ...
        notice.className = 'muted history-window-notice';
        notice.textContent = `Großer Lauf: ${entities.length.toLocaleString()} von ${totalEntities.toLocaleString()} Individuen als schnelles Arbeitsfenster geladen.`;
        target.querySelector('.history-desk-head > div')?.append(notice);
      }
      ...
      const selectEntity = async id => {
        ...
        if(!entity){const dossier=await fetch(`/api/entity/${id}`,{cache:'no-store'}).then(response=>response.json());if(dossier.error)return;entity={...dossier,born_at:dossier.born,energy:0,start_energy:0,parents:dossier.parents||[],partners:[],alive:dossier.alive};byId.set(id,entity);entities.push(entity)}
        window.eveV04.selectEntity(id);
        if (!entity._detailsLoaded) {
          detail.innerHTML='<div class="amoeba-detail-empty panel"><div>Amöben-Dossier wird geladen …</div></div>';
          try {
            const result=await fetch(`/api/history/entity/${id}?tick=${Number(currentTick)||0}`,{cache:'no-store'}).then(response=>response.json());
            if(result.error)throw Error(result.error);
            Object.assign(entity,result.entity,{_detailsLoaded:true});
            const knownEvents=new Set(activityEvents.map(event=>`${event.tick}:${event.kind}:${event.entity_id||0}`));
            for(const event of result.events||[]){const key=`${event.tick}:${event.kind}:${event.entity_id||0}`;if(!knownEvents.has(key)){activityEvents.push(event);knownEvents.add(key)}}
          } catch(error) {
            detail.innerHTML=`<p class="error">Dossier konnte nicht geladen werden: ${safe(error.message||error)}</p>`;
            return;
          }
        }
        ...
        detail.querySelectorAll('[data-open-lineage]').forEach(button=>button.onclick=()=>this.showLineageById(entity.id));
      ...
      target._selectEntity=selectEntity;
  ...
  function renderChronicle(root,data){
    ...
    if(data.summary_only){const hall=root.querySelector('.chronicle-hall');if(hall)hall.innerHTML='<header><div><small>EVE · HALL OF LIFE</small><h3>Amöbenrekorde auf Abruf</h3></div></header><p class="muted">Die teure Vollauswertung aller Amöben wird nicht mehr beim Öffnen der Chronik ausgeführt.</p>';const summary=root.querySelector('.chronicle-summary>div:last-child');if(summary)summary.innerHTML='<small>ARCHIVMODUS</small><strong>Schneller Index</strong>'}
  ...
  let lineageStarted = false, lineageRefreshing = false, selectedLineageId = null;
  async function initLineage(force=false, focusId=null) {
    const requestedFocus=focusId??selectedLineageId;
    if(!requestedFocus){const root=document.querySelector('#lineage-workspace');root.className='lineage-loading panel';root.innerHTML='<div><b>Erst Amöbe in der Historie wählen.</b><p>Öffne dort ein Amöben-Dossier und wähle „Stammbaum“.</p></div>';return}
    if (lineageStarted && !force) return;
    if (lineageRefreshing) return;
    lineageStarted = true; lineageRefreshing = true;
    const root = document.querySelector('#lineage-workspace');
    try {
      const focus = requestedFocus;
      const url = focus ? `/api/lineage?focus=${focus}&ancestors=2&descendants=2&limit=400` : '/api/lineage?ancestors=2&descendants=2&limit=400';
      const data = await fetch(url, { cache: 'no-store' }).then(response => response.json());
      if (data.error) throw Error(data.error);
      const signature=`${data.current_tick||0}:${(data.entities||[]).length}:${(data.entities||[]).filter(entity=>entity.alive).length}`;
      if(force&&root.dataset.lineageSignature===signature){root.querySelector('[data-lineage-refresh-state]')?.replaceChildren(`aktuell · ${new Date().toLocaleTimeString('de-DE')}`);return}
      const restore=force&&root._lineageSnapshot?root._lineageSnapshot():null;
      renderLineage(root, data, restore);
      root.dataset.lineageSignature=signature;
    } catch (error) {
      lineageStarted = false;
      root.innerHTML = `<p class="error">Stammbaum konnte nicht geladen werden: ${String(error.message || error)}</p>`;
    } finally { lineageRefreshing=false }
  }

  function renderLineage(root, data, restore=null) {
    ...
    const clusterSvg = entities.length < 45 ? '' : [...clusterBuckets.entries()].map(([key,bucket])=>{const x=bucket.reduce((sum,e)=>sum+xFor(e.born),0)/bucket.length,y=bucket.reduce((sum,e)=>sum+yFor(e),0)/bucket.length,r=8+Math.min(30,Math.sqrt(bucket.length)*6);return `<g class="lineage-cluster" data-cluster="${key}" data-lineage-tip="Verdichtete Linie|${bucket.length} ${bucket.length===1?'Amöbe':'Amöben'}|Generation ${bucket[0].generation}" transform="translate(${x} ${y})"><circle r="${r}"/><text text-anchor="middle" y="3">${bucket.length}</text><title>${bucket.length} Amöben · Generation ${bucket[0].generation}</title></g>`}).join('');
    ...
    root.innerHTML=`<header class="lineage-toolbar"><div><small>EVE · FAMILIENFENSTER</small><h3>${entities.length} von ${(data.total_entities||entities.length).toLocaleString()} Amöben · ${maxGeneration+1} Generationen · Tick 0–${maxTick.toLocaleString()}</h3></div><div class="lineage-actions"><label>Amöbe # <input data-lineage-focus-input type="number" min="1" value="${data.focus_id||''}" style="width:7em"></label><button type="button" data-lineage-load-focus>Familie laden</button><label>Linienwert <select data-lineage-metric><option value="children">Nachkommen</option><option value="life">Lebensdauer</option><option value="genome">Genomgröße</option></select></label><button data-lineage-zoom="out">−</button><button data-lineage-zoom="in">+</button><button data-lineage-reset>Fenster</button><span class="lineage-zoom-label">Übersicht</span><button type="button" data-lineage-refresh>↻</button><small data-lineage-refresh-state>manuell</small><button type="button" data-analysis-focus>⛶ Vergrößern</button></div></header><div class="lineage-legend"><span><i class="alive"></i>lebend</span><span><i class="extinct"></i>ausgestorben</span><span><i class="mutation"></i>Mutation</span><strong>Begrenzt auf 4 Vorfahren- und 3 Nachfahrenstufen · Klick: Verwandtschaft</strong></div><div class="lineage-milestone-bar">${milestoneBar}</div><section class="lineage-compare"><div data-compare-slot="a"><small>VERGLEICH A</small><b>Amöbe wählen</b></div><div class="lineage-common"><small>LETZTER GEMEINSAMER VORFAHR</small><b>—</b></div><div data-compare-slot="b"><small>VERGLEICH B</small><b>Umschalt-Klick</b></div><button data-compare-clear>× Vergleich leeren</button></section><div class="lineage-stage"><svg class="lineage-graph ${entities.length>=45?'dense':''}" viewBox="0 0 ${width} ${height}" role="img" aria-label="Evolutionärer Stammbaum"><defs><pattern id="lineage-grid" width="80" height="74" patternUnits="userSpaceOnUse"><path d="M80 0H0V74" fill="none" stroke="rgba(65,157,222,.065)"/></pattern><filter id="lineage-glow"><feGaussianBlur stdDeviation="3" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter></defs><rect class="lineage-grid" width="100%" height="100%" fill="url(#lineage-grid)"/>${generationBands}<g class="lineage-events">${eventSvg}${milestoneSvg}</g><g class="lineage-edges">${edgeSvg}</g><g class="lineage-nodes">${nodeSvg}</g><g class="lineage-cluster-nodes">${clusterSvg}</g><path class="lineage-playhead" data-lineage-tip="Ausgewählter Zeitpunkt|Tick ${maxTick}" d="M130 35V${height-25}"/></svg><div class="lineage-tooltip" hidden></div></div><aside class="lineage-inspector"><div><small>EVOLUTIONÄRER FOKUS</small><h3>Wähle eine Amöbe.</h3><p>Das geladene Fenster enthält nur ihre nähere Familie.</p></div></aside>`;
    root.querySelector('[data-lineage-focus-input]')?.closest('label')?.remove();
    root.querySelector('[data-lineage-load-focus]')?.remove();
    const familyScope=root.querySelector('.lineage-legend strong');if(familyScope)familyScope.textContent='Eltern und Großeltern · Kinder und Enkel · Klick: Verwandtschaft';
    bindAnalysisFocus(root);
    root.addEventListener('click',event=>{const node=event.target.closest?.('[data-lineage-id]');if(node)window.eveV04.selectEntity(Number(node.dataset.lineageId))},true);
    ...
    const base={x:0,y:0,w:width,h:height}; let view=restore?.view?{x:restore.view.x*width,y:restore.view.y*height,w:restore.view.w*width,h:restore.view.h*height}:{...base}, returnView=null, drag=null, moved=false, focusId=null, compare=[];
    ...
    root.querySelector('[data-lineage-zoom="in"]').onclick=()=>zoom(.72);root.querySelector('[data-lineage-zoom="out"]').onclick=()=>zoom(1/.72);root.querySelector('[data-lineage-reset]').onclick=()=>{returnView=null;root.querySelector('[data-compare-clear]').click();animateViewTo({...base})};root.querySelector('[data-lineage-refresh]').onclick=()=>initLineage(true,data.focus_id);
    ...
    svg.querySelectorAll('.lineage-cluster').forEach(cluster=>cluster.addEventListener('click',event=>{event.stopPropagation();const members=clusterBuckets.get(cluster.dataset.cluster)||[],cx=members.reduce((sum,e)=>sum+xFor(e.born),0)/members.length,cy=members.reduce((sum,e)=>sum+yFor(e),0)/members.length;view={x:cx-base.w/6,y:cy-base.h/6,w:base.w/3,h:base.h/3};clamp();applyView();inspector.innerHTML=`<div><small>VERDICHTETE LINIE</small><h3>${members.length} Amöben · Generation ${members[0]?.generation??'—'}</h3><p>Die Gruppe wurde aufgelöst. Klicke nun ein Individuum für ihre Verwandtschaft an.</p></div>`}));clamp();applyView();root._lineageSnapshot=()=>({view:{x:view.x/base.w,y:view.y/base.h,w:view.w/base.w,h:view.h/base.h},focusId,compare:[...compare],metric:root.querySelector('[data-lineage-metric]').value});root.querySelector('[data-lineage-refresh-state]').textContent=`manuell · ${new Date().toLocaleTimeString('de-DE')}`;if(data.focus_id&&byId.has(data.focus_id))focus(byId.get(data.focus_id),true);else if(restore?.focusId&&byId.has(restore.focusId))focus(byId.get(restore.focusId),true);else if(restore?.compare?.length){restore.compare.filter(id=>byId.has(id)).forEach(addCompare)}
  }


  document.querySelectorAll('.eve-nav button').forEach(button => button.addEventListener('click', () => {
    const view=button.dataset.view;
    window.eveV04.setView(view);
    if(!selectedEntityId)return;
    if(view==='genome')window.eveV04.showGenomeById(selectedEntityId);
    if(view==='lineage'){selectedLineageId=selectedEntityId;initLineage(true,selectedEntityId)}
    if(view==='history')requestAnimationFrame(()=>{const row=document.querySelector(`[data-entity-row="${selectedEntityId}"]`);if(row)row.click();else document.querySelector('#entities')?._selectEntity?.(selectedEntityId)});
  }));
  ...
  window.dispatchEvent(new Event('eve:v04-ready'));
  loadRuns().then(()=>{const requested=new URLSearchParams(location.search).get('view')||sessionStorage.getItem('eve-open-view');if(requested){sessionStorage.removeItem('eve-open-view');window.eveV04.setView(requested)}}).catch(() => { runSelect.innerHTML = '<option>Run-Katalog nicht erreichbar</option>'; });
})();
