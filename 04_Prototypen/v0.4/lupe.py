...
const select=document.querySelector('#snap'),box=document.querySelector('#entities'),metaBox=document.querySelector('#meta'),contactCanvas=document.querySelector('#contacts'),chartCanvas=document.querySelector('#chart'),replayBox=document.querySelector('#replay'),replaySlider=document.querySelector('#replay-slider'),envSelect=document.querySelector('#env-entity'),esc=v=>String(v).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));let replayFrames=[],replayIndex=0,replayTimer=null,environmentEvents=[],activityEvents=[],timelinePoints=[],timelineMilestones=[],timelineHover=null,timelineViewport={start:0,end:0},timelinePan=null,currentTick=0,selection={mode:'all',start:0,end:0},dragStart=null,visibleIds=new Set(),runIsActive=false,timelineRefreshing=false;const genomeCache=new Map(),names=['Tom','Erna','Ada','Bruno','Clara','Dario','Emmi','Fritz','Greta','Hugo','Ida','Juri','Karla','Lino','Maja','Nils','Olga','Piet','Rosa','Sam','Tilda','Uwe','Vera','Willi','Xenia','Yara','Zeno','Alma','Ben','Cleo','Dora','Enno'],nameOf=id=>names[(id-1)%names.length]+(id>names.length?' '+(Math.floor((id-1)/names.length)+1):''),amoeba=e=>`${esc(e.name||nameOf(e.id))}${e.ram_position===undefined?'':` <span class="muted">@RAM ${e.ram_position}</span>`}`;
...
function genomeFragments(g){if(!g)return[];const byId=new Map(g.nodes.map(n=>[n.id,n])),segmented=g.nodes.length&&g.nodes.every(n=>Number.isInteger(Number(n.segment))&&Number(n.segment)>0),groups=new Map();if(segmented)g.nodes.forEach(n=>{const key=Number(n.segment),items=groups.get(key)||[];items.push(n.id);groups.set(key,items)});else{const links=new Map(g.nodes.map(n=>[n.id,new Set()]));g.edges.forEach(p=>{links.get(p.source)?.add(p.target);links.get(p.target)?.add(p.source)});const unseen=new Set(byId.keys());while(unseen.size){const first=unseen.values().next().value,stack=[first],ids=[];unseen.delete(first);while(stack.length){const id=stack.pop();ids.push(id);for(const next of links.get(id)||[])if(unseen.delete(next))stack.push(next)}groups.set(groups.size+1,ids)}}const out=[];for(const[segment,rawIds]of groups){const ids=rawIds.sort((a,b)=>a-b),idSet=new Set(ids),partEdges=g.edges.filter(p=>segmented?idSet.has(p.source):idSet.has(p.source)&&idSet.has(p.target)),kinds=ids.map(id=>byId.get(id).kind),writes=ids.filter(id=>byId.get(id).kind==='MEM_WRITE'),social=writes.some(id=>{const ports=new Set(g.edges.filter(p=>p.target===id).map(p=>p.target_port));return['offset','slot','value'].every(p=>ports.has(p))}),caps=[];if(social)caps.push('Partnersuche & Handshake');else if(writes.length)caps.push('unvollständiges MEM_WRITE');if(kinds.includes('RAM_READ'))caps.push('RAM-Erkundung');if(kinds.includes('RAM_WRITE'))caps.push('RAM-Markierung');if(!caps.length)caps.push('unbenanntes Segment');out.push({segment,ids,edges:partEdges.length,kinds:[...new Set(kinds)].sort(),label:caps.join(' + '),social})}return out.sort((a,b)=>a.segment-b.segment)}
function fragmentView(g){const fs=genomeFragments(g),social=fs.some(f=>f.social);return `<h3 data-tip title="Explizite Segmente werden bei der Geburt als erbliche Einheiten kombiniert; ausgehende Anschlusskanten können neu gekoppelt werden">Erbliche Genomsegmente · ${fs.length}</h3><div class="fragment-list">${fs.map(f=>`<div class="fragment"><b>Segment ${f.segment} · ${esc(f.label)}</b><br><span class="muted">${f.ids.length} F · ${f.edges} ausgehende P · Knoten ${f.ids.join(', ')}</span><br><span class="source">${esc(f.kinds.join(' · '))}</span></div>`).join('')}</div><div class="entity-meta">Strukturell ausführbares MEM_WRITE: <b class="${social?'':'missing'}">${social?'vorhanden':'fehlt · Membranwert maximal 9'}</b>. Das garantiert noch keinen gültigen Partner oder sozialen Erfolg.</div>`}
