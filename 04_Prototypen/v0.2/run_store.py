"""Versionierte, read-only veröffentlichbare Run-Datenhaltung für EVE-Alife v0.2."""
...
class RunStore:
    ...
    def finish(self, simulation: Any, end_reason: str) -> None:
        if end_reason not in {"natural_extinction", "tick_limit_reached"}:
            raise ValueError(f"Unbekannter Endgrund: {end_reason}")
        status = "extinct" if end_reason == "natural_extinction" else "completed"
        population = sum(entity.alive for entity in simulation.entities.values())
        self.db.execute(
