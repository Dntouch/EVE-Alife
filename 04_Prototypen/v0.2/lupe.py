                ...
                with readonly_connection(database) as db:
                    if path == "/api/status":
                        ...
                        if path == "/api/activity":
                            events = [event for event in events if event["kind"] in {"birth", "death", "birth_rejected", "mem_write", "mem_write_rejected", "reproduction_cost", "ram_write", "genome_created"} or (event["kind"] == "ram_read" and event.get("reward", 0) > 0)]
                        else:
